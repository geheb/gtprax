...
internal sealed class NodeGenerator : INodeGenerator
{
    private static bool IsRazorPage(Type type) => type != null && typeof(PageModel).IsAssignableFrom(type);
    private readonly Dictionary<Type, NodeDto> _nodes = [];
    ...
    public NodeDto? Find(string returnUrl)
    {
    ...
    public NodeDto GetNode<T>() where T : class => GetNode(typeof(T));

    public NodeDto GetNode(Type pageType)
    {
    ...
    private static bool TryMatch(string routeTemplate, string requestPath)
    {
        ...
        return matcher.TryMatch(requestPath, new());
    }

    private static void GenerateHierarchy(Dictionary<Type, NodeDto> entries)
    {
        if (entries.Count < 1)
        {
            return;
    ...
    private static NodeDto CreateBreadcrumbNode(Type pageType, NodeAttribute node, string pagePath, AuthorizeAttribute? auth)
    {
        ...
        return new NodeDto(pageType, pagePath, node.IsDefault, node.Title)
        {
