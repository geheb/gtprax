...
public sealed class NodeGenerator
{
    ...
    {
        if (!IsRazorPage(pageType)) throw new InvalidOperationException($"'{pageType.FullName}' isn't a Razor page");
    ...
    public void Add(Assembly assembly)
    {
        foreach (var type in assembly.GetTypes().Where(t => IsRazorPage(t)))
        {
            ...
            if (node != null) _nodes.Add(node.Key, node);
        }
    ...
    private static void GenerateHierarchy(Dictionary<Type, Node> entries)
    {
        ...
        foreach (var entry in entries.Values)
        {
            ...
            if (!entries.TryGetValue(fromKey, out var node)) throw new InvalidOperationException($"No node exists that has a '{fromKey.FullName}' as a key. Make sure that Razor page has a {nameof(NodeAttribute)}");
    ...
    private static string ExtractRazorPage(Type pageType)
    {
        ...
        int pagesIndex = fullName != null ? fullName.IndexOf($".{RazorPagesRootDirectory}.") : -1;
        if (pagesIndex == -1) throw new InvalidOperationException($"The full name {fullName} doesn't contain '{RazorPagesRootDirectory}'");
    ...
    private static Node? CreateBreadcrumbNode(Type type)
    {
        ...
        if (attr.FromPage != null && !IsRazorPage(attr.FromPage)) throw new InvalidOperationException($"FromPage '{attr.FromPage.FullName}' isn't a Razor Page");

        var auth = type.GetCustomAttribute<AuthorizeAttribute>();

        //if no title is given, then fallback to type name (razor page name).
        if (string.IsNullOrWhiteSpace(attr.Title)) attr.Title = type.Name.Replace("Page", string.Empty);

        string page = ExtractRazorPage(type);
